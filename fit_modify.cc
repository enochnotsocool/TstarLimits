...
// ---------------------------------------------------------
Fitter::Fitter(int AdditionRNDSeed) : BCModel()
{
  data_=0; data2_=0; data3_=0;
  functionIntegral_=0;
  printlevel_=3;
  strategy_=0; //strategy_=2;   // unknown
  minuit_.SetErrorDef(0.5); // likelihood
  rand_=new TRandom3(31415+AdditionRNDSeed);
  parameters_=0;
  poiIndex_=-1;
  ...
  parRangeSet_=false;
  useMCMC_=false;
  MCMCGetTRandom3()->SetSeed(31415+AdditionRNDSeed);
}

// ---------------------------------------------------------
Fitter::Fitter(int AdditionRNDSeed, TH1D* data, TH1D* data2, TH1D* data3, integral_ptr_t functionIntegral, int maxpar) : BCModel(), minuit_(maxpar)
{
  data_=data; data2_=data2; data3_=data3;
  functionIntegral_=functionIntegral;
  printlevel_=3;
  strategy_=0; //strategy_=2;
  minuit_.SetErrorDef(0.5); // likelihood
      ...
      {       
          if(it->second>0) {
              ...
              { 
                  ...
                  pars[poiIndex_]=poiBestFit_; 
                  // here we need to use the best-fit value in order to probe the posterior likelihood close to its maximum  
